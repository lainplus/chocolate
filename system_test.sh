...
tmp=$(cd $(mktemp -d ${TMPDIR:-/tmp}/chocolate-system-test-XXXXXX); pwd -P)
...
try "no arguments"
	./chocolate 2> /dev/null || code=$?
	assert $code 1

try "no input"
	./chocolate echo "vroom" 2> /dev/null || code=$?
	assert $code 1

try "reload and clear options with no utility to run"
	./chocolate -r -c 2> /dev/null || code=$?
	assert $code 1

try "empty input"
	echo "" | ./chocolate echo 2> /dev/null || code=$?
	assert $code 1

try "no regular files provided as input"
	mkdir $tmp/dir1
	ls $tmp | ./chocolate echo 2> /dev/null || code=$?
	rmdir $tmp/dir1
	assert $code 1
...
try "spacebar triggers utility"
	...
	tmux send-keys -t $tsession:0 \
	    "ls $tmp/file2 | ./chocolate -p cp $tmp/file2 $tmp/file1" C-m ; zz
...
try "exec a command using one-shot option"
	setup
	ls $tmp/file* | ./chocolate -zp cat $tmp/file2 >$tmp/exec.out 2>$tmp/exec.err &
	bgpid=$! ; zz
...
try "fail to exec a command using one-shot option"
	setup
	ls $tmp/file* | ./chocolate -z /usr/bin/false_X >$tmp/exec.out 2>$tmp/exec.err &
	bgpid=$! ; zz
	wait $bgpid || assert "$?" "1"

try "exec a command using one-shot option exit code from child"
	setup
	ls $tmp/file* | ./chocolate -z sh -c 'exit 4' &
	bgpid=$! ; zz
	wait $bgpid || assert "$?" "4"
...
try "restart a server when a file is modified using one-shot option"
	...
	if [ $(uname) == 'Linux' ]; then
		...
		ls $tmp/file2 | ./chocolate -rz nc -l -U $tmp/nc.s >> $tmp/exec.out &
		bgpid=$! ; zz
...
try "exec a command in non-intertive mode"
	setup
	ls $tmp/file* | ./chocolate -n tty >$tmp/exec.out &
	bgpid=$! ; zz
	kill -INT $bgpid
	wait $bgpid || assert "$?" "130"
...
try "exec a command as a background task and ensure stdin is closed"
	setup
	ls $tmp/file* | ./chocolate -r sh -c 'test -t 0; echo $?; kill $$' >$tmp/exec.out &
	bgpid=$! ; zz
	kill -INT $bgpid
	wait $bgpid || assert "$?" "130"
...
try "exec a command as a background task, and verify that read from stdin doesn't complain"
	setup
	ls $tmp/file* | ./chocolate -r sh -c 'read X' 2>$tmp/exec.err &
	bgpid=$! ; zz
	kill -INT $bgpid
	wait $bgpid || assert "$?" "130"
	assert "$(cat $tmp/exec.err)" ""
...
try "exec single shell utility and exit when a file is added to an implicit watch path"
	setup
	ls $tmp/file* | ./chocolate -dp sh -c 'echo ping' >$tmp/exec.out 2>$tmp/exec.err \
	    || true &
	bgpid=$! ; zz
	touch $tmp/newfile
	wait $bgpid || assert "$?" "130"
	assert "$(cat $tmp/exec.out)" "ping"
	assert "$(cat $tmp/exec.err)" "chocolate: directory altered"

try "exec single shell utility and exit when a subdirectory is added"
	setup
	ls -d $tmp | ./chocolate -dp sh -c 'echo ping' >$tmp/exec.out 2>$tmp/exec.err \
	    || true &
	bgpid=$! ; zz
	mkdir $tmp/newdir
	wait $bgpid || assert "$?" "130"
...
try "do nothing when a file not monitored is changed in directory watch mode"
	setup
	ls $tmp/file2 | ./chocolate -dp echo "changed" >$tmp/exec.out 2>$tmp/exec.err &
	bgpid=$! ; zz
	echo "123" > $tmp/file1
	kill -INT $bgpid
	wait $bgpid || assert "$?" "130"
...
try "exec utility when a file is written by Vim in directory watch mode"
	setup
	ls $tmp/file* | ./chocolate -dp echo "changed" >$tmp/exec.out 2>$tmp/exec.err &
	bgpid=$! ; zz
	vim -e -s -u NONE -N \
	    -c ":r!date" \
	    -c ":wq" $tmp/file1 ; zz
...
try "exec utility when a file is opened for write and then closed"
	setup
	echo "---" > $tmp/file1
	ls $tmp/file* | ./chocolate -p echo "changed" > $tmp/exec.out &
	bgpid=$! ; zz
	: > $tmp/file1 ; zz
	kill -INT $bgpid
	wait $bgpid || assert "$?" "130"
...
try "exec single utility when an entire stash of files is reverted"
	...
	cd - > /dev/null ; zz
	ls $tmp/*.h | ./chocolate -p echo "changed" > $tmp/exec.out &
	bgpid=$! ; zz
	cd $tmp
	git checkout *.h -q
	cd - > /dev/null ; zz
...
try "restart a server when a file is modified"
	...
	ls $tmp/file2 | ./chocolate -r tail -f $tmp/file1 2> /dev/null > $tmp/exec.out &
	bgpid=$! ; zz
...
try "ensure that all shell subprocesses are terminated in restart mode"
	...
	SCRIPT
	chmod +x $tmp/go.sh
	ls $tmp/file2 | ./chocolate -r sh -c "$tmp/go.sh" 2> /dev/null > $tmp/exec.out &
	bgpid=$! ; zz
	kill -INT $bgpid ; zz
...
try "exit with no action when restart and dirwatch flags are combined"
	setup
	echo "started." > $tmp/file1
	ls $tmp/file* | ./chocolate -rd tail -f $tmp/file1 2> /dev/null > $tmp/exec.out &
	bgpid=$! ; zz
	assert "$(cat $tmp/exec.out)" "started."
	touch $tmp/newfile
	kill -INT $bgpid
...
try "exec single shell utility when two files change simultaneously"
	...
	ls $tmp/file* | ./chocolate -p sh -c 'echo ping' > $tmp/exec.out &
	bgpid=$! ; zz
...
try "exec single shell utility on startup and when a file is changed"
	setup
	ls $tmp/file* | ./chocolate sh -c 'printf ping' > $tmp/exec.out &
	bgpid=$! ; zz
	echo 456 >> $tmp/file1 ; zz
	kill -INT $bgpid
	wait $bgpid || assert "$?" "130"
...
try "exec a command if a file is made executable"
	setup
	ls $tmp/file* | ./chocolate -p echo /_ > $tmp/exec.out &
	bgpid=$! ; zz
...
try "ensure watches operate on a running executable"
	setup
	cp /bin/sleep $tmp/
	ls $tmp/sleep | ./chocolate -rs "echo 'vroom'; $tmp/sleep 30" \
...
try "exec a command using the first file to change"
	setup
	ls $tmp/file* | ./chocolate -p cat /_ > $tmp/exec.out &
	bgpid=$! ; zz
	echo 456 > $tmp/file1 ; zz
	kill -INT $bgpid
	wait $bgpid || assert "$?" "130"
...
try "exec single shell utility using utility substitution"
	setup
	ls $tmp/file1 $tmp/file2 | ./chocolate -p file /_ > $tmp/exec.out &
	bgpid=$! ; zz
	echo 456 >> $tmp/file2; zz
	kill -INT $bgpid
	wait $bgpid || assert "$?" "130"
...
try "watch and exec a program that is overwritten"
	...
	echo $tmp/script | ./chocolate -p $tmp/script $tmp/file1 > $tmp/exec.out &
	bgpid=$! ; zz
	cat > $tmp/script <<-EOF
	#!/bin/sh
	echo vroom
...
try "exec an interactive utility when a file changes"
	setup
	ls $tmp/file* | ./chocolate -p sh -c 'tty | cut -c1-8' 2> /dev/null > $tmp/exec.out &
	bgpid=$! ; zz
	echo 456 >> $tmp/file2 ; zz
	kill -INT $bgpid
	wait $bgpid || assert "$?" "130"
...
try "exec a command using shell option"
	setup
	ls $tmp/file* | ./chocolate -ps 'file $0; exit 2' >$tmp/exec.out 2>$tmp/exec.err &
	bgpid=$! ; zz
	echo 456 >> $tmp/file2 ; zz
	kill -INT $bgpid
	wait $bgpid || assert "$?" "130"
...
try "exec a command as a background task"
	setup
	(ls $tmp/file* | ./chocolate -ps 'echo terminating; kill $$' >$tmp/exec.out 2>$tmp/exec.err &)
	zz
	echo 456 >> $tmp/file2 ; zz
	assert "$(cat $tmp/exec.err)" ""
...
try "respond to events that occur while the utility is running"
	setup
	ls $tmp/file* | ./chocolate -a sh -c 'echo "vroom"; sleep 0.5' > $tmp/exec.out &
	bgpid=$! ; zz
	echo "123" > $tmp/file1
	sleep 1
	kill -INT $bgpid
